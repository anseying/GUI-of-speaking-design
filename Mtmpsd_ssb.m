...
for i=2:fn-1;                                   %这种平滑尽可能大的保留了本身频谱的能量特性
%     abs_frame_f(:,i)=.25*abs_frame(:,i-1)+.5*abs_frame(:,i)+.25*abs_frame(:,i+1);
    abs_frame_f(:,i)=.375*abs_frame(:,i-1)+.25*abs_frame(:,i)+.375*abs_frame(:,i+1);
end;                             %与平均平滑不同
...
%     per_PSD_f(:,i)=.25*per_PSD(:,i-1)+.5*per_PSD(:,i)+.25*per_PSD(:,i+1);
    per_PSD_f(:,i)=.375*per_PSD(:,i-1)+.25*per_PSD(:,i)+.375*per_PSD(:,i+1);
end;                                         
per_PSD=per_PSD_f;
...
% 谱减求取增益因子     每一帧，每一帧的求取增益因子
%没有进行比较判断，而是即使加上判断也是会进行计算的，索性改变方式，
%直接全部计算，按照符合条件的进行赋值
for k=1:fn;

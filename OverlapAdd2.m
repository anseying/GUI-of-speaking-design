...
for i=1:FrameNum                             % 按式(10-2-1)重叠相加法把数据叠接合成
    start=(i-1)*ShiftLen+1;                   % 计算i帧在sig中的起始位置       重叠相加；
    spec=Spec(:,i);                              % 取第i帧的频谱相位
...
%Y is the signal reconstructed signal from its spectrogram. 
% X is a matrix
%with each column being the fft of a segment of signal. 
% A is the phase
...
% W is the window length of time
%domain segments if not given the length is assumed to be twice as long as
%fft window length.
% S is the shift length of the segmentation process ( for
